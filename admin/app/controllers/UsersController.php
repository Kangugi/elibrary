...
class UsersController extends SecureController{
	...
	function view($rec_id = null, $value = null){
		...
		$fields = array("username", 
			...
			"date_updated", 
			"user_id");
		$db->where("users.username", get_active_user('username') );
		if($value){
			$db->where($rec_id, urldecode($value)); //select record based on field name
		}
		else{
			$db->where("users.user_id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("username","password","phone","email","status");
			...
			if($this->validated()){
				...
				if($rec_id){
					$this->set_flash_msg("User added successfully", "success");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("username","phone","status");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'username' => 'required',
		...
		$db->where("users.username", get_active_user('username') );$db->where("users.user_id", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("users.user_id", $arr_rec_id, "in");
		$db->where("users.username", get_active_user('username') );
